...
def sigmoid(x):
    new_x = []
    for item in x:
        new_x.append (1 / (1 + math.exp(-item)))
    return new_x
...
def train(features, labels, weights, lr, iters):
    ...
    for i in range(iters):
        ...
        # Log Progress
        if i % 1000 == 0:
            predicted_probs = predict(features, weights)
            predicted_labels = classify(predicted_probs)
            # print accuracy(predicted_labels, labels)

            print "iter: "+str(i) + " cost: "+str(cost) + " w norm square: "+str(np.dot(weights, weights))
...
weights, cost_history = train(data, labels, weights, ALPHA, 10000)
